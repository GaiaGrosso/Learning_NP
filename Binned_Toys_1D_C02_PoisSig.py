...
for i in range(1):
    ID='C_NP0_patience'+str(patience)+'_bins'+str(N_bins)+'_bkg'+str(NevBkg)+'_sig'+str(NevSig)+'_epochs'+str(total_epochs)+'_latent'+str(latentsize)+'_wclip'+str(weight_clipping)
...
#    TOY_ID = 'Toy1D_binned_'+str(NevtMC)+'_'+str(NevBkg)+'_'+str(NevSig)+'_'+sys.argv[2]
    ...
    # create sample                                                                                                                                
    featureData = np.random.rand(NevBkg, 1)                      
    ...
    for i in range(ref_bins):
        ...
        ref_integral = uniform.cdf(ref_xmin+ref_bin*(i+1), loc=ref_xmin, scale=ref_range)-uniform.cdf(ref_xmin+ref_bin*(i), loc=ref_xmin, scale=ref_range)
