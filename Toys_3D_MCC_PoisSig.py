...
for i in range(1):
    ID='/M_NP'+str(NP)+'_patience'+str(patience)+'_ref'+str(NevMC)+'_bkg'+str(NevBkg)+'_sig'+str(N_Exp_Sig)+'_epochs'+str(total_epochs)+'_latent'+str(latentsize)+'_wclip'+str(weight_clipping)
...
#    TOY_ID = 'Toy1D_binned_'+str(NevtMC)+'_'+str(NevBkg)+'_'+str(NevSig)+'_'+sys.argv[2]
    ...
    featureMC3 = np.random.rand(NevMC,1)
    ...
    featureMC = np.concatenate((featureMC, featureMC3), axis=1)
    ...
    # data sample                                                                                                                                                      
    featureData1 = np.random.exponential(scale=0.125, size=(NevBkg,1))
    featureData2 = np.random.rand(NevBkg, 1)
    featureData3 = np.random.rand(NevBkg, 1)
    #featureData2 = np.random.normal(loc=0.5, scale=0.1, size=NevBkg)

    if NP==1:
        featureSig1 = np.random.normal(scale=0.02, loc=0.8, size=(NevSig, 1))
    elif NP==2:
        featureSig1 = NP2_gen(size=NevSig, seed=seed)
        featureSig1 = np.expand_dims(featureSig1, axis=1)
    elif NP==3:
        featureSig1 = np.random.normal(scale=0.02, loc=0.2, size=(NevSig, 1))
    #featureSig1 = np.random.normal(loc=0.8, scale=0.02, size=NevSig)
    featureSig2 = np.random.rand(NevSig, 1)
    featureSig3 = np.random.rand(NevSig, 1)
    ...
    featureData3 = np.concatenate((featureData3, featureSig3), axis =0)
    featureData1 = featureData1.reshape((NevBkg+NevSig, 1))
    featureData2 = featureData2.reshape((NevBkg+NevSig, 1))
    featureData3 = featureData3.reshape((NevBkg+NevSig, 1))
    ...
    featureData = np.concatenate((featureData, featureData3), axis = 1)
