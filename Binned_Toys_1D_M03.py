...
def MyModel(nInput, latentsize):
    ...
    dense  = Dense(latentsize, input_shape=(nInput,), activation='sigmoid', W_constraint = WeightClip(10.*weight_clipping))(inputs) #prova del 30/10/18
...
for i in range(1):
    ID='M03_BKG_patience'+str(patience)+'_bins'+str(N_bins)+'_bkg'+str(NevBkg)+'_sig'+str(NevSig)+'_epochs'+str(total_epochs)+'_latent'+str(latentsize)+'_wclip'+str(weight_clipping)
