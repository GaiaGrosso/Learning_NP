...
# parameters defining the experiment: ####################################
OUTPUT_DIRECTORY = './'+str(mass_cut)+'_inf/'

config_json = {
    "N_Bkg"   : 4000,                                                                                                                          
    "output_directory": OUTPUT_DIRECTORY,
    "shape_nuisances_generation": [csec_nuisances_data['trim_ZToMuMu_M_50_120']],
    "shape_nuisances_reference": [csec_nuisances_reference['trim_ZToMuMu_M_50_120']],
    "shape_nuisances_sigma": [csec_nuisances_sigma['trim_ZToMuMu_M_50_120']],
    "norm_nuisances_generation": 0,
    "norm_nuisances_reference":  0,
    "norm_nuisances_sigma":      np.sqrt(0.026**2 + 2*0.015**2 + 0.002**2 + 0.01**2),
    ...
    "epochs": 300000,
    "patience": 10000,
    "BSMarchitecture": [5, 5, 5, 5, 1],
    "BSMweight_clipping": 3.,
    "correction": "SHAPE",
    "shape_dictionary_list": [csec_trim_ZToMuMu_parNN],
    "Mcut": 400,
}

ID = str(config_json["Mcut"])+'_inf/RunAll_Nref' + "ALL"
ID+='_Nbkg'+str(config_json["N_Bkg"]) + '_patience'+str(config_json["patience"])
ID+='_epochs'+str(config_json["epochs"])+'_arc'+str(config_json["BSMarchitecture"]).replace(', ', '_').replace('[', '').replace(']', '')+'_wclip'+str(config_\
json["BSMweight_clipping"])
########################################################################
